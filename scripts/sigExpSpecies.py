...
parser.add_argument('-i', '--input', help='input fasta file of the CDS', required=True)
...
def extractSigProteins(sequences, significantFile, output):
    ...
    significantSeqRecords = [SeqRecord(s.seq.translate(), id=s.id, description=s.description) for s in sequences if s.id in significantIDs]
